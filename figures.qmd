...
cyano <- read_excel("Data/PMODEL_Data for Steff Figs.xlsx", sheet = "Fig_Ch2", skip = 2) |>
  ...
  select(source, lake,
         ...
         chl, secchi, humic, doc, color, pri_proa, bac, coldfish, fishabun)
```
...
### Setup the data and the base plots
...
```{r}
g1 <- g0 +
  geom_point(aes(y = chl), na.rm = TRUE) +
  stat_smooth(aes(y = chl), method = "lm", se = FALSE, na.rm = TRUE, colour = "black") +
...
> **Questions**
> 
> - Which HUMIC are clear/coloured? (0/3 = clear, 1 = coloured?)
> - This has extra data compare to example figure... okay? (see especially low values of TP)
...
g2 <- g0 +
  ...
  geom_point(aes(y = secchi, shape = humic_fct), na.rm = TRUE) +
  stat_smooth(aes(y = secchi), method = "lm", se = FALSE, na.rm = TRUE, colour = "black") +
...
> **Questions**
> 
> - What is the regression line in this figure?
> - In the example it doesn't seem to line up with the data, here I just used
>   the data present

```{r}
g3 <- g0 +
  geom_point(aes(y = doc), na.rm = TRUE) +
  stat_smooth(aes(y = doc), method = "lm", se = FALSE, na.rm = TRUE, colour = "black") +
...
```{r}
g4 <- g0 +
  geom_point(aes(y = color), na.rm = TRUE) +
  stat_smooth(aes(y = color), method = "lm", se = FALSE, na.rm = TRUE, colour = "black") +
...
```{r}
g5 <- g0 +
  geom_point(aes(y = pri_proa), na.rm = TRUE) +
  stat_smooth(aes(y = pri_proa), method = "lm", se = FALSE, na.rm = TRUE, colour = "black") +
...
```{r}
g6 <- g0 +
  geom_point(aes(y = bac), na.rm = TRUE) +
  stat_smooth(aes(y = bac), method = "lm", se = FALSE, na.rm = TRUE, colour = "black") +
...
(g1 + g2) / (g3 + g4) / (g5 + g6) + plot_annotation(tag_levels = "A", tag_prefix = "(", tag_suffix = ")")
```
