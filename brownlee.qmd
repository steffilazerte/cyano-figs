...
g0 <- ggplot(d, aes(x = rm, colour = year, shape = year, fill = year)) +
  ...
  scale_x_reverse(name = "Location (RM)", limits = c(345, 283), breaks = seq(340, 290, -10)) +
...
g00 <- g0 +
  ...
  # Outflow
  annotate(geom = "text", label = "Outflow", x = 284, y = 0.5) +
  annotate(geom = "segment", arrow = arrow(type = "closed", length = unit(2, "mm")),
...
g3 <- g0 +
  ...
  scale_y_continuous(limits = c(0, 50), name = bquote("Chlorophyl"~(mu*g/L)))


g00 / g3 / g1 / g2 + plot_annotation(tag_levels = "A") +
